...
from Configs.config import settings, Task, task_types
...
async def handle_random(deal_id: int, cont_id: int) -> str:
    ...
    """
    logger.info(
        "Endpoint catch /CarexRandom", extra={"deal_id": deal_id, "cont_id": cont_id}
    )
    task_type = task_types.CAREX_RANDOM  # Выбирается ключ для задачи, которое потом отправится в очередь Redis
    ...
    async with redis.Redis.from_url(settings.REDIS_URL) as r:
        ...
        logger.info(
            f"Task added to queue: {settings.TASKS}", extra={"task": task.model_dump()}
        )
