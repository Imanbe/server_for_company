...
def setup_logging():
    ...
    app_logger = logging.getLogger("company_app")
    ...
    app_file_handler = RotatingFileHandler(
        "Logs/company_main.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10 MB
    )
    ...
    debug_logger = logging.getLogger("company_debug")
    debug_logger.setLevel(logging.DEBUG)
    debug_logger.handlers.clear()

    # Обработчик для подробного файла
    debug_file_handler = RotatingFileHandler(
        "Logs/company_debug.log", maxBytes=10 * 1024 * 1024, backupCount=5
    )
    ...
    root_logger.setLevel(
        logging.DEBUG
    )  # Устанавливаем минимальный уровень для сторонних библиотек
